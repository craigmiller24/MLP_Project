...
def train_multinomial_logistic_regression_gpu(X_train, X_test, Y_train, Y_test, num_epochs=100, lr=0.01):
...
def train_decision_tree_gpu(X_train_torch, X_test_torch, Y_train_torch, Y_test_torch, feature_names):
    # Move tensors to CPU for conversion to numpy/pandas
    # (Decision Tree is from sklearn which requires numpy arrays)
    X_train_cpu = X_train_torch.cpu()
    X_test_cpu = X_test_torch.cpu()
    Y_train_cpu = Y_train_torch.cpu()
    Y_test_cpu = Y_test_torch.cpu()
    
    # Convert PyTorch tensors to Pandas DataFrame using the feature names
    X_train_df = pd.DataFrame(X_train_cpu.numpy(), columns=feature_names)
    X_test_df = pd.DataFrame(X_test_cpu.numpy(), columns=feature_names)

    # Convert labels to numpy arrays
    Y_train = Y_train_cpu.numpy()
    Y_test = Y_test_cpu.numpy()
